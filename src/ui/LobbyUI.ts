...
export class LobbyUI {
                    ...
                    <div class="stitch-auth-panel" role="dialog" aria-modal="true" aria-labelledby="stitch-auth-title">
                        ...
                        <form class="stitch-auth-form" data-auth-form="register">
                            <div class="stitch-auth-form-grid stitch-auth-form-grid--register">
                                ...
                                <div class="stitch-auth-verify-block" data-auth-verify-block="email">
                                    ...
                                    <div class="stitch-auth-verify-row" data-auth-verify-row="email">
    ...
    this.root
      ...
      .forEach((element) => {
        ...
        element.style.display =
          !status.loggedIn && usingRegisterMode && enabled ? "grid" : "none";
      });

    this.root
      .querySelectorAll<HTMLButtonElement>("[data-auth-send-code]")
      .forEach((button) => {
        const cooldown = this.authCodeCooldowns.email;
        button.disabled = this.authBusy || cooldown > 0;
        button.textContent =
          this.authBusyAction === "emailCode"
            ? "发送中..."
            : cooldown > 0
              ? `${cooldown}s 后重发`
              : "发送邮箱验证码";
  ...
  private async handleSendRegisterVerificationCode() {
    ...
    if (this.authCodeCooldowns.email > 0) {
      return;
    }
  ...
  private startAuthCodeCooldown(seconds: number) {
    this.authCodeCooldowns.email = Math.max(0, Math.floor(seconds));
    if (
      this.authCodeCooldownTimer === null &&
      this.authCodeCooldowns.email > 0
    ) {
      this.authCodeCooldownTimer = window.setInterval(() => {
        this.authCodeCooldowns.email = Math.max(
          0,
          this.authCodeCooldowns.email - 1,
        );
  ...
  private getAuthModeForField(fieldName: AuthFieldName): AuthMode {
    return fieldName.startsWith("authLogin") ? "login" : "register";
  }
