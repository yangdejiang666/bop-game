...
export class MatchmakingUI {
  ...
  constructor(options: MatchmakingUIOptions) {
    ...
    const progressLabelEl = this.root.querySelector<HTMLSpanElement>(
      "[data-match-progress-label]",
    );
  ...
  startExternal(modeId: LobbyModeId) {
    ...
    this.stageTextEl.textContent = "等待服务器匹配状态...";
    this.statusTextEl.textContent = "已进入后端匹配队列，正在同步实时进度。";
  }
  ...
  private tickSearching(now: number) {
    if (now >= this.nextIncreaseAtMs) {
      ...
      this.currentPlayers = Math.min(
        this.targetPlayers,
        this.currentPlayers + burst,
      );
      this.nextIncreaseAtMs = now + this.getIncreaseIntervalMs();
    }
  ...
  private enterConfirming(now: number) {
    this.stage = "confirming";
    this.progress = 1;
    this.etaSeconds = 0;
  ...
  private syncUI() {
    ...
    this.etaTextEl.textContent =
      this.etaSeconds > 0 ? `预计 ${this.etaSeconds} 秒` : "预计 1 秒";
  }
